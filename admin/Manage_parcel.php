...
{
 $delete_sql="DELETE from parcel where Pid =' ".$_REQUEST['Pid_remove']."'";
 $del_result=mysqli_query($conn,$delete_sql);
 if($del_result)
 {
 	echo"<script> alert('record deleted');window.location='Manage_parcel.php';</script>";
 }
 else{
 	echo"<script> alert('record deleted failed !!');window.location='Manage_parcel.php';</script>";
 }
 }
?>
<!DOCTYPE html>
...
<tr bgcolor=#E1C16E>
        ...
        <th>Parcel Address</th>
          ...
          <th>Remove</th>
                ...
                <td>
                <?php echo $row['PdeliveryDT']; ?>
                </td>
                <td>
                <?php echo $row['PdeliveryBY']; ?>
                </td>
               <td align="center">
					        		<a href="Manage_parcel.php?Pid_remove=<?php echo $row ['Pid']?>"><img src  = "icons8-delete-16.png" width="30%" height="50%">
						</img></a>
						</td>
